...
- Получаем:  
  - $y_{\text{obs}}$ — массив наблюдений для обучения (после порога),  
...
**Серийный интервал** $ w_s $ получаем из гамма‑распределения (по заданным mean/sd), дискретизируем по дням и нормируем $ \sum_{s=1}^{S} w_s = 1 $.
...
\mathbb{E}[y_t]=\mu_t,\quad \operatorname{Var}(y_t)=\mu_t+\frac{\mu_t^2}{\alpha}.
$$
...
На обучении NUTS не максимизирует правдоподобие, а сэмплирует из апостериора, пропорционального *likelihood* $ \times $ *priors*.
...
$$
\mathrm{MAE}=\frac{1}{n}\sum_t \lvert y_t-\hat y_t\rvert,\qquad
\mathrm{MAPE}=\frac{100\%}{n}\sum_t \frac{\lvert y_t-\hat y_t\rvert}{\max(y_t,1)}.
$$
